...
async fn test_circuit_breaker_config_presets() {
    ...
    assert_eq!(conservative_config.recovery_timeout, Duration::from_secs(120));
}
...
async fn test_error_context_creation() {
    ...
    assert!(matches!(context.suggested_action, SuggestedAction::NoAction));
}
...
async fn test_suggested_actions() {
    ...
    assert!(matches!(switch_action, SuggestedAction::SwitchProvider { .. }));
...
async fn test_error_recovery_manager_multiple_errors() {
    ...
    let errors = vec![
        (AiLibError::RateLimitExceeded("Rate limit 1".to_string()), ErrorType::RateLimit),
        (AiLibError::RateLimitExceeded("Rate limit 2".to_string()), ErrorType::RateLimit),
        (AiLibError::NetworkError("Network error".to_string()), ErrorType::Network),
        (AiLibError::TimeoutError("Timeout".to_string()), ErrorType::Timeout),
    ];
