...
impl CohereAdapter {
    ...
    pub fn new() -> Result<Self, AiLibError> {
        let api_key = std::env::var("COHERE_API_KEY").map_err(|_| AiLibError::AuthenticationError("COHERE_API_KEY environment variable not set".to_string()))?;
        let base_url = std::env::var("COHERE_BASE_URL").unwrap_or_else(|_| "https://api.cohere.ai".to_string());
        Ok(Self {
    ...
    pub fn with_transport(transport: HttpTransport, api_key: String, base_url: String) -> Self {
        Self { transport: transport.boxed(), api_key, base_url }
    }
    ...
    pub fn with_transport_ref(transport: DynHttpTransportRef, api_key: String, base_url: String) -> Self {
        Self { transport, api_key, base_url }
    }
    ...
    fn convert_request(&self, request: &ChatCompletionRequest) -> serde_json::Value {
        ...
        if let Some(temp) = request.temperature {
            chat_body["temperature"] = serde_json::Value::Number(serde_json::Number::from_f64(temp.into()).unwrap());
        }
        if let Some(max_tokens) = request.max_tokens {
            chat_body["max_tokens"] = serde_json::Value::Number(serde_json::Number::from(max_tokens));
        }
    ...
    fn parse_response(&self, response: serde_json::Value) -> Result<ChatCompletionResponse, AiLibError> {
        ...
        let content = if let Some(c) = response.get("choices") {
            c[0]["message"]["content"].as_str().map(|s| s.to_string())
        ...
        // Construct a minimal ChatCompletionResponse
        let choice = Choice {
            index: 0,
            message: Message { role: Role::Assistant, content: content.clone() },
            finish_reason: None,
        };
...
impl ChatApi for CohereAdapter {
    async fn chat_completion(&self, request: ChatCompletionRequest) -> Result<ChatCompletionResponse, AiLibError> {
        ...
        let chat_body = body;
    let resp_chat = self.transport.post_json(&url_chat, Some(headers.clone()), chat_body.clone()).await;
        ...
        let response_json = match resp_chat {
            ...
            Err(_) => {
                ...
                let prompt = if !request.messages.is_empty() {
                    request.messages.iter().map(|m| format!("{}: {}", match m.role { Role::System => "System", Role::User => "User", Role::Assistant => "Assistant" }, m.content)).collect::<Vec<_>>().join("\n")
                } else { String::new() };
                ...
                if let Some(temp) = request.temperature {
                    gen_body["temperature"] = serde_json::Value::Number(serde_json::Number::from_f64(temp.into()).unwrap());
                }
                if let Some(max_tokens) = request.max_tokens {
                    gen_body["max_tokens"] = serde_json::Value::Number(serde_json::Number::from(max_tokens));
                }

                self.transport.post_json(&url_generate, Some(headers.clone()), gen_body.clone()).await?
            }
        };
                                    ...
                                    while let Some(boundary) = find_event_boundary(&buffer) {
                                        ...
                                        if let Ok(event_text) = std::str::from_utf8(&event_bytes) {
                                            if let Some(parsed) = parse_sse_event(event_text) {
                                                match parsed {
                                                    Ok(Some(chunk)) => {
                                                        if tx.send(Ok(chunk)).is_err() { return; }
                                                    }
                                                    Ok(None) => return, // [DONE]
        ...
        tokio::spawn(async move {
            ...
            for chunk in chunks {
                ...
                let delta = crate::api::ChoiceDelta {
                    index: 0,
                    delta: crate::api::MessageDelta { role: Some(crate::types::Role::Assistant), content: Some(chunk.clone()) },
                    finish_reason: None,
                };
    ...
    let response: serde_json::Value = self.transport.get_json(&url, Some(headers)).await?;
        ...
        Ok(response["models"].as_array()
            .unwrap_or(&vec![])
            .iter()
            .filter_map(|m| m["id"].as_str().map(|s| s.to_string()).or_else(|| m["name"].as_str().map(|s| s.to_string())))
            .collect())
    }
