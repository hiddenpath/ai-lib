...
impl MistralAdapter {
    pub fn new() -> Result<Self, AiLibError> {
        ...
        let base_url = std::env::var("MISTRAL_BASE_URL").unwrap_or_else(|_| "https://api.mistral.ai".to_string());
        let boxed = HttpTransport::new().boxed();
        Ok(Self { transport: boxed, api_key, base_url })
    }
    ...
    fn convert_request(&self, request: &ChatCompletionRequest) -> serde_json::Value {
        ...
        if let Some(temp) = request.temperature { body["temperature"] = serde_json::Value::Number(serde_json::Number::from_f64(temp.into()).unwrap()); }
        if let Some(max_tokens) = request.max_tokens { body["max_tokens"] = serde_json::Value::Number(serde_json::Number::from(max_tokens)); }
        body
    }
    ...
    fn parse_response(&self, response: serde_json::Value) -> Result<ChatCompletionResponse, AiLibError> {
        let choices = response["choices"].as_array()
            ...
            .enumerate()
            .map(|(index, choice)| {
                let message = choice["message"].as_object().ok_or_else(|| AiLibError::ProviderError("Invalid choice format".to_string()))?;
                let role = match message["role"].as_str().unwrap_or("user") { "system" => Role::System, "assistant" => Role::Assistant, _ => Role::User };
                let content = message["content"].as_str().unwrap_or("").to_string();
                Ok(Choice { index: index as u32, message: Message { role, content }, finish_reason: choice["finish_reason"].as_str().map(|s| s.to_string()) })
            })
            .collect::<Result<Vec<_>, AiLibError>>()?;

        let usage = response["usage"].as_object().ok_or_else(|| AiLibError::ProviderError("Invalid response format: usage not found".to_string()))?;
        let usage = Usage { prompt_tokens: usage["prompt_tokens"].as_u64().unwrap_or(0) as u32, completion_tokens: usage["completion_tokens"].as_u64().unwrap_or(0) as u32, total_tokens: usage["total_tokens"].as_u64().unwrap_or(0) as u32 };

        Ok(ChatCompletionResponse { id: response["id"].as_str().unwrap_or_default().to_string(), object: response["object"].as_str().unwrap_or_default().to_string(), created: response["created"].as_u64().unwrap_or(0), model: response["model"].as_str().unwrap_or_default().to_string(), choices, usage })
    }
}
...
fn find_event_boundary(buffer: &[u8]) -> Option<usize> {
    ...
    while i < buffer.len().saturating_sub(1) {
        if buffer[i] == b'\n' && buffer[i + 1] == b'\n' { return Some(i + 2); }
        if i < buffer.len().saturating_sub(3) && buffer[i] == b'\r' && buffer[i+1] == b'\n' && buffer[i+2] == b'\r' && buffer[i+3] == b'\n' { return Some(i + 4); }
        i += 1;
    }
    None
}
...
fn parse_sse_event(event_text: &str) -> Option<Result<Option<ChatCompletionChunk>, AiLibError>> {
    for line in event_text.lines() {
        ...
        if line.starts_with("data: ") {
            let data = &line[6..];
            if data == "[DONE]" { return Some(Ok(None)); }
...
fn parse_chunk_data(data: &str) -> Result<Option<ChatCompletionChunk>, AiLibError> {
    let json: serde_json::Value = serde_json::from_str(data).map_err(|e| AiLibError::ProviderError(format!("JSON parse error: {}", e)))?;
    ...
    if let Some(arr) = json["choices"].as_array() {
        for (index, choice) in arr.iter().enumerate() {
            ...
            let role = delta.get("role").and_then(|v| v.as_str()).map(|r| match r { "assistant" => Role::Assistant, "user" => Role::User, "system" => Role::System, _ => Role::Assistant });
            let content = delta.get("content").and_then(|v| v.as_str()).map(|s| s.to_string());
            let md = MessageDelta { role, content };
            let cd = ChoiceDelta { index: index as u32, delta: md, finish_reason: choice.get("finish_reason").and_then(|v| v.as_str()).map(|s| s.to_string()) };
            choices_vec.push(cd);
        }
    }
...
fn split_text_into_chunks(text: &str, max_len: usize) -> Vec<String> {
    ...
    while start < s.len() {
        ...
        let mut cut = end;
        if end < s.len() {
            if let Some(pos) = text[start..end].rfind(' ') { cut = start + pos; }
        }
        if cut == start { cut = end; }
        let chunk = String::from_utf8_lossy(&s[start..cut]).to_string();
        chunks.push(chunk);
        start = cut;
        if start < s.len() && s[start] == b' ' { start += 1; }
    }
    chunks
}
...
impl ChatApi for MistralAdapter {
    async fn chat_completion(&self, request: ChatCompletionRequest) -> Result<ChatCompletionResponse, AiLibError> {
        ...
        if let Some(key) = &self.api_key { headers.insert("Authorization".to_string(), format!("Bearer {}", key)); }

    let response: serde_json::Value = self.transport.post_json(&url, Some(headers), provider_request).await?;
    ...
    async fn chat_completion_stream(&self, request: ChatCompletionRequest) -> Result<Box<dyn Stream<Item = Result<ChatCompletionChunk, AiLibError>> + Send + Unpin>, AiLibError> {
        ...
        let client = client_builder.build().map_err(|e| AiLibError::ProviderError(format!("Client error: {}", e)))?;
        ...
        for (k, v) in headers.clone() { req = req.header(k, v); }
                    ...
                    tokio::spawn(async move {
                        ...
                        while let Some(item) = stream.next().await {
                            match item {
                                ...
                                Err(e) => { let _ = tx.send(Err(AiLibError::ProviderError(format!("Stream error: {}", e)))); break; }
                            }
                        }
                    });
        ...
        let text = finished.choices.get(0).map(|c| c.message.content.clone()).unwrap_or_default();
        let (tx, rx) = mpsc::unbounded_channel();
        tokio::spawn(async move {
            let chunks = split_text_into_chunks(&text, 80);
            for chunk in chunks {
                let delta = ChoiceDelta { index: 0, delta: MessageDelta { role: Some(Role::Assistant), content: Some(chunk.clone()) }, finish_reason: None };
                let chunk_obj = ChatCompletionChunk { id: "simulated".to_string(), object: "chat.completion.chunk".to_string(), created: 0, model: finished.model.clone(), choices: vec![delta] };
