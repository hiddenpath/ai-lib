...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("DeepSeek feature demo: interceptors + unified_sse");
    ...
    let req = ChatCompletionRequest::new(
        model.clone(),
        vec![Message { role: Role::User, content: Content::new_text("Say hello in one line."), function_call: None }]
    );

    // Interceptor pipeline wraps a non-streaming call
