...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if std::env::var("GROQ_API_KEY").is_ok() {
        ...
        let request = ChatCompletionRequest::new(
            ...
            vec![Message {
                role: Role::User,
                content: Content::Text("Say 'Retry test successful!' exactly.".to_string()),
                function_call: None,
            }],
        )
        .with_max_tokens(10);
    ...
    if std::env::var("GROQ_API_KEY").is_ok() {
        ...
        let request = ChatCompletionRequest::new(
            ...
            vec![Message {
                role: Role::User,
                content: Content::Text("Test invalid model".to_string()),
                function_call: None,
            }],
        )
        .with_max_tokens(10);
    ...
    if std::env::var("GROQ_API_KEY").is_ok() {
        ...
        let request = ChatCompletionRequest::new(
            ...
            vec![Message {
                role: Role::User,
                content: Content::Text("Network error test".to_string()),
                function_call: None,
            }],
        )
        .with_max_tokens(10);
