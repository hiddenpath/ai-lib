...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match transport.get::<serde_json::Value>("https://api.openai.com/v1/models", Some(headers)).await {
        Ok(response) => {
            let model_count = response["data"].as_array().map(|arr| arr.len()).unwrap_or(0);
    ...
    match transport.post::<serde_json::Value, serde_json::Value>(
